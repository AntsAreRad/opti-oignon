#!/usr/bin/env python3
"""S277 -- the answer-verification UI half (the SvelteKit lot): the standalone
surface the user drives over the already-built S272 route, the
container-provable half of a labelled host-assured browser lot (the s248 /
s253 / s269 / s275 precedent).

S272 landed the per-answer verification route
(``POST /api/claims/verify-answer``, returning
``AnswerVerificationResultSchema``): a single per-user POST that takes a batch
of (claim, source) pairs directly, hands them to
``claim_aggregation.make_answer_verifier(client) -> verify_answer(pairs)``
(S271, fail-secure), and returns the aggregate verdict plus the per-pair
results. Unlike the S274 citation-verify route, this route performs no
extraction: the caller supplies the pairs, so the result carries no echoed
pairs (the caller already holds them). The UI half was deferred there. S277
lands that front: one typed API client (``api/answerVerification.ts`` over
``/api/claims/verify-answer``), one standalone component
(``AnswerVerifier.svelte``: an editable list of (claim, source) pairs, a model
Select, a Verify action, and the structured aggregate shown alongside the
per-pair verdicts aligned with the submitted pairs), and one standalone page
(``routes/verify-answer/+page.svelte``) that renders it.

The component is added ALONGSIDE -- it does not edit ClaimVerifier.svelte (the
S269 UI), CitationVerifier.svelte (the S275 UI), routes/claims/+page.svelte,
routes/verify-citations/+page.svelte, Sidebar.svelte (the S270 / S276 nav), or
any other pinned UI surface, and it adds NO cartography row (the S274 / S275
...
surface is a proven ZERO -- the eleventh twin of the S266 register. A sidebar
nav entry is a deliberate follow-on lot (the S270 / S276 precedent), not part
of this lot. The live browser run is host-assured
(ANSWER_VERIFICATION_UI_E2E_S277.md); this suite proves the container-provable
artifacts.
...
hex) -- the s174 / s248 / s253 / s269 / s275 idiom -- not by a browser;
esbuild validation of the new TS / Svelte is an Apply-phase step, not a pytest
concern. Seven families:

 1. The TS client -- existence, the base-client wiring (apiPost), the
    AnswerVerificationResult interface mirroring the backend aggregate schema
    (verdict, ok, reason, and the per-pair results, with NO echoed pairs), the
    per-pair interface, the input ClaimSourcePair interface, the verdict union
    mirroring the role taxonomy, the endpoint, the verify function, and the
    pairs batch / optional model.
 2. The component -- AnswerVerifier: the ds-primitive imports, the client /
    model-source wiring, the behaviour surface (an editable (claim, source)
    pair list, the Verify action, the in-flight disable, the CONFIRMED
    server-truth posture), and the structured display (the aggregate verdict /
    reason and the per-pair verdicts aligned with the submitted pairs), the
    --oo-* tokens (no raw hex), and Svelte block balance.
 3. The page route -- routes/verify-answer/+page.svelte: the component
    imported and rendered. A new standalone page, NOT an edit of a pinned
    surface.
 4. The runbook -- ANSWER_VERIFICATION_UI_E2E_S277.md: existence, status
 ...
 5. The seams the UI is the client of -- source pins on the premises (the
    S272 route and its single endpoint / aggregate schema / no-mode-gate, the
    S271 aggregator and its make_answer_verifier, the S267 role taxonomy, the
    base HTTP client's apiPost, the ds primitives, the selectedModel store,
    the models client), so a later edit that removes a premise turns this
    suite red instead of letting the UI rot. Green on the pristine S276 tree
    by design.
...
Red-before discipline: on the pristine S276 tree (no UI client, no component,
no page, no runbook) every family-1..4 file pin FAILS and the family-7
new-file ASCII pins FAIL -- the read helpers return empty strings so absence
is a bare AssertionError, never a collection error -- while every family-5
seam pin and the family-6 not-a-tool / the family-7 suite-self negatives pass
(they pin pre-existing premises and empty-string-true negatives). Document
pins read through a whitespace-flattening helper (the s221 / s238 lesson) so
reflow cannot break them; source pins stay raw. This suite imports no package
code, so no ollama chain is touched.
"""
...
# New frontend source landed by S277.
CLIENT = FE / "lib" / "api" / "answerVerification.ts"
COMPONENT = FE / "lib" / "components" / "panels" / "AnswerVerifier.svelte"
PAGE = FE / "routes" / "verify-answer" / "+page.svelte"

# Docs.
RUNBOOK = ROOT / "ANSWER_VERIFICATION_UI_E2E_S277.md"

# Seam premises (pre-existing; green on the pristine S276 tree by design).
ROUTE = ROOT / "opti_oignon" / "api" / "routes_answer_verification.py"
...
class TestAnswerVerificationClient:
    ...
    def test_aggregate_interface_mirrors_schema(self):
        s = _read(CLIENT)
        assert "export interface AnswerVerificationResult" in s
        # Mirrors AnswerVerificationResultSchema: the aggregate plus the
        # per-pair results. There is NO echoed pairs field (S272 takes the
        # pairs from the caller and does not re-emit them).
        for field in ("verdict", "ok", "reason", "results"):
    ...
    def test_input_pair_interface_present(self):
        s = _read(CLIENT)
        # The submitted (claim, source) pair shape (the batch input).
        assert "export interface ClaimSourcePair" in s
        assert "claim" in s
        assert "source" in s
    ...
    def test_verify_function_and_endpoint(self):
        s = _read(CLIENT)
        assert "export async function verifyAnswer" in s
        assert "apiPost" in s
        assert "/api/claims/verify-answer" in s

    def test_carries_pairs_and_optional_model(self):
        s = _read(CLIENT)
        assert "pairs" in s
...
class TestAnswerVerifierComponent:
    def test_exists(self):
    ...
    def test_wires_client(self):
        s = _read(COMPONENT)
        assert "from '$lib/api/answerVerification'" in s
        assert "verifyAnswer" in s
    ...
    def test_displays_per_pair(self):
        s = _read(COMPONENT)
        # The per-pair results rendered alongside the submitted pairs. The
        # component iterates the per-pair results the route returns.
        assert "results" in s
        assert "{#each" in s

    def test_no_raw_hex(self):
        raw = _read(COMPONENT)
        assert raw, "AnswerVerifier.svelte absent"
...
class TestVerifyAnswerPageRoute:
    def test_exists(self):
        assert PAGE.exists()
    ...
    def test_imports_and_renders_component(self):
        s = _read(PAGE)
        assert (
            "import AnswerVerifier from "
            "'$lib/components/panels/AnswerVerifier.svelte'" in s
        )
        assert "<AnswerVerifier" in s
...
class TestRunbook:
    def test_exists_and_titled(self):
        text = _read(RUNBOOK)
        assert text.startswith("# ANSWER_VERIFICATION_UI_E2E_S277")
        assert len(text) > 4000

    def test_status_and_discipline(self):
        text = _flat(_read(RUNBOOK))
        assert "written at S277" in text
...
class TestSeamRouteAndPipeline:
    def test_route_single_endpoint_and_schema(self):
        ...
        assert "@answer_verification_router.post(" in src
        assert '"/verify-answer"' in src
        assert "AnswerVerificationResultSchema" in src

    def test_route_aggregate_schema_fields(self):
        src = _read(ROUTE)
        assert "class AnswerVerificationResultSchema" in src
        # The aggregate schema mirrors the aggregation result: NO echoed pairs.
        for field in ("verdict", "ok", "reason", "results"):
