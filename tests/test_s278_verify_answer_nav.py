#!/usr/bin/env python3
"""S278 -- the /verify-answer discoverability surface.

The container-provable half of a labelled, host-assured nav walk: a single
sidebar nav entry that makes the S277 ``/verify-answer`` page discoverable,
on the S276 ``/verify-citations`` nav idiom (itself the S270 ``/claims`` idiom,
itself the S248 ``/notes`` idiom). A proven-ZERO add-only lot -- it edits one
existing source (the sidebar component) without flipping any canonical pin, and
adds a test and a host-assured runbook; the failing-name register stays
byte-identical (the twelfth twin of the S266 register) and the version holds at
3.13.0.

Seven families, the S253 / S269 / S270 / S276 idiom:

1. the nav entry -- the ``/verify-answer`` object (href, label, icon) inside
   the ``navLinks`` array, placed after the ``/verify-citations`` entry (RED on
   the pristine S277 tree, GREEN on the delivered tree);
2. the page seam -- the S277 ``/verify-answer`` page still renders
   ``AnswerVerifier`` (a design-green premise, true on pristine);
3. the nav idiom -- the sidebar declares ``navLinks``, carries the
   ``/verify-citations`` entry, and uses the ``Icon`` primitive (design-green
   premises);
...
VERIFY_ANSWER_PAGE = FRONTEND_SRC / "routes" / "verify-answer" / "+page.svelte"
SPEC = ROOT / "FRONTEND_REDESIGN_SPEC.md"
RUNBOOK = ROOT / "VERIFY_ANSWER_NAV_E2E_S278.md"
...
# ---------------------------------------------------------------------------
# Family 1 -- the /verify-answer nav entry (RED on the pristine S277 tree)
# ---------------------------------------------------------------------------


class TestNavEntry:
    def test_verify_answer_href_present(self):
        assert "'/verify-answer'" in _read(SIDEBAR)

    def test_verify_answer_label_present(self):
        assert "label: 'Answers'" in _read(SIDEBAR)

    def test_verify_answer_entry_well_formed(self):
        src = _read(SIDEBAR)
        pattern = (
            r"\{\s*href:\s*'/verify-answer',\s*"
            r"label:\s*'Answers',\s*icon:\s*'[a-z0-9-]+'\s*\}"
        )
        assert re.search(pattern, src) is not None

    def test_verify_answer_entry_within_navlinks(self):
        region = _navlinks_region(_read(SIDEBAR))
        assert "'/verify-answer'" in region

    def test_verify_answer_placed_after_verify_citations(self):
        src = _read(SIDEBAR)
        assert ("'/verify-answer'" in src) and (
            src.find("'/verify-answer'") > src.find("'/verify-citations'")
        )


# ---------------------------------------------------------------------------
# Family 2 -- the page seam (design-green: S277 already ships the page)
# ---------------------------------------------------------------------------


class TestPageSeam:
    def test_verify_answer_page_renders_verifier(self):
        assert "AnswerVerifier" in _read(VERIFY_ANSWER_PAGE)
...
class TestNavIdiom:
    ...
    def test_sidebar_verify_citations_entry_present(self):
        src = _read(SIDEBAR)
        assert ("'/verify-citations'" in src) and ("label: 'Citations'" in src)
...
class TestRunbook:
    def test_exists_and_titled(self):
        assert _read(RUNBOOK).startswith("# VERIFY_ANSWER_NAV_E2E_S278")

    def test_status_and_discipline(self):
        text = _flat(_read(RUNBOOK))
        assert "written at S278" in text
