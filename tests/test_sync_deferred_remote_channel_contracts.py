#!/usr/bin/env python3
"""Sync deferred ledger and remote-channel contracts: provenance only.

The deferred ledger is the quarantine between the wire and the local set:
its list surface exposes provenance and never a record body, approval
re-enters the engine's full verify-gate-apply seam, refusal applies
nothing, and the republish ceremony answers honestly when this device
cannot sign. The remote channel keeps its one-gesture revocations: a
grant disable, a revoke and an unpair all drop the device's live
streaming sessions. This suite pins that behavior:

  * DL1 -- the deferred list serialises exactly the provenance fields
    and never leaks a record body;
  * DL2 -- approval refuses missing keys with a 400 before the engine,
    maps an unknown key to 404, coerces the keys to strings, and passes
    the engine's honest summary through unchanged;
  * DL3 -- refusal removes the entry, applies nothing, and answers with
    provenance plus the removal flag;
  * DL4 -- republish maps a signing failure to an honest 503 and a
    success to the republished count;
  * DL5 -- disabling the remote-chat grant kills the device's live
    sessions, enabling does not, and a non-boolean flag is ignored;
  * DL6 -- revoke gates an unknown peer with a 404, flips the grant,
    and reports the number of killed sessions;
  * DL7 -- unpair removes the peer, kills its sessions best-effort even
    when the kill fails, and answers 404 on a second unpair.
...
_PROVENANCE_KEYS = {
    "kind", "record_id", "origin_device", "peer_id",
    "clock", "deferred_at", "last_offered_at",
}
...
def test_deferred_list_exposes_provenance_only():
    """DL1: the list carries exactly the provenance fields, never a body."""
    env = _load()
    try:
        cfg = env.cfg
        cfg.deferred_entries = [_entry()]
        out = env.mod.sync_deferred_list()
        assert out["count"] == 1
        item = out["deferred"][0]
        assert set(item) == _PROVENANCE_KEYS
...
def test_approve_requires_keys_maps_unknown_and_passes_through():
    """DL2: missing keys are a 400 before the engine; the summary is verbatim."""
    env = _load()
    try:
        cfg = env.cfg
        _expect_refusal(400, env.mod.sync_deferred_approve, {})
        _expect_refusal(400, env.mod.sync_deferred_approve, {"kind": "skill"})
        assert cfg.engine.approve_calls == []

        cfg.approve_error = cfg.exc.deferred_missing()
        exc = _expect_refusal(
            404, env.mod.sync_deferred_approve,
            {"kind": "skill", "record_id": "nope"},
        )
        assert "No pending record" in exc.detail

        cfg.approve_error = None
        cfg.approve_result = {"approved": False, "refused": True}
        out = env.mod.sync_deferred_approve({"kind": 5, "record_id": 7})
        assert out == {"approved": False, "refused": True}
        assert cfg.engine.approve_calls[-1] == ("5", "7")
    finally:
        env.restore()


def test_refuse_removes_the_entry_and_applies_nothing():
    """DL3: refusal removes the entry with provenance only; nothing applies."""
    env = _load()
    try:
        cfg = env.cfg
        cfg.refuse_entry = _entry()
        out = env.mod.sync_deferred_refuse(
            {"kind": "skill", "record_id": "r1"}
        )
        assert cfg.engine.refuse_calls == [("skill", "r1")]
        assert cfg.engine.approve_calls == []
        assert out["removed"] is True
        assert out["kind"] == "skill"
        assert "SECRET" not in json.dumps(out)

        cfg.refuse_error = cfg.exc.deferred_missing()
        _expect_refusal(
            404, env.mod.sync_deferred_refuse,
            {"kind": "a", "record_id": "b"},
        )
    finally:
        env.restore()
...
def test_republish_is_an_honest_503_or_a_count():
    """DL4: a signing failure is a named 503; success reports the count."""
    env = _load()
    try:
        cfg = env.cfg
        cfg.republish_error = RuntimeError("no signing key")
        exc = _expect_refusal(503, env.mod.sync_republish)
        assert "Signing unavailable" in exc.detail

        cfg.republish_error = None
        cfg.republish_count = 4
        out = env.mod.sync_republish()
        assert out == {"republished": 4}
        assert len(cfg.engine.republish_calls) == 2
    finally:
        env.restore()


def test_grant_disable_kills_live_sessions_enable_does_not():
    """DL5: disable is a live revoke; enable is not; non-booleans are ignored."""
    env = _load()
    try:
        cfg = env.cfg
        cfg.store.add(_peer())
        out = env.mod.sync_set_remote_chat_grant(
            "peer-a", {"enabled": False}
        )
        assert cfg.store.grant_calls[-1] == ("peer-a", False)
        assert cfg.kill_calls == ["peer-a"]
        assert out["remote_chat_enabled"] is False

        out = env.mod.sync_set_remote_chat_grant("peer-a", {"enabled": True})
        assert cfg.store.grant_calls[-1] == ("peer-a", True)
        assert cfg.kill_calls == ["peer-a"]
        assert out["remote_chat_enabled"] is True

        before = list(cfg.store.grant_calls)
        out = env.mod.sync_set_remote_chat_grant(
            "peer-a", {"enabled": "yes", "rag": True}
        )
        assert cfg.store.grant_calls == before
        assert cfg.store.rag_calls[-1] == ("peer-a", True)
        assert out["rag_subgrant"] is True
    finally:
        env.restore()


def test_revoke_flips_counts_kills_and_gates_unknown():
    """DL6: revoke is 404 for an unknown peer, else flip plus counted kill."""
    env = _load()
    try:
        cfg = env.cfg
        _expect_refusal(404, env.mod.sync_revoke_remote_chat, "ghost")
        assert cfg.store.grant_calls == []

        cfg.store.add(_peer())
        cfg.kill_result = 2
        out = env.mod.sync_revoke_remote_chat("peer-a")
        assert out == {
            "peer_id": "peer-a",
            "revoked": True,
            "killed_sessions": 2,
...
def test_unpair_kills_best_effort_and_answers_honestly():
    """DL7: unpair kills sessions best-effort and is honest on repeats."""
    env = _load()
    try:
        cfg = env.cfg
        cfg.store.add(_peer())
        out = env.mod.sync_unpair("peer-a")
        assert out == {"peer_id": "peer-a", "removed": True}
        assert cfg.kill_calls == ["peer-a"]
        assert "peer-a" not in cfg.store.peers

        _expect_refusal(404, env.mod.sync_unpair, "peer-a")

        cfg.store.add(_peer("peer-b"))
        cfg.kill_error = RuntimeError("streaming module down")
        out = env.mod.sync_unpair("peer-b")
        assert out == {"peer_id": "peer-b", "removed": True}
        cfg.kill_error = None
    finally:
        env.restore()
