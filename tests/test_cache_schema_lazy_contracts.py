...
def test_cl7_the_ledger_is_empty_and_still_read():
    """Supersedes cl6: its control asked the ledger to be
    non-empty, and the ledger is empty since the package import opens nothing."""
    guard, restore = _guard()
    try:
        for name in ("response_cache.db", "semantic_cache.db"):
            assert name not in guard.LEDGER["databases"], f"{name} no longer opens at import, so it must come off"
        ...
        guard.LEDGER = {**guard.LEDGER, "databases": frozenset({"response_cache.db"})}
        assert "response_cache.db" in " ".join(guard.stale_entries(seen)), "the ledger is read against the observation"
    finally:
        restore()
