#!/usr/bin/env python3
"""S276 -- the /verify-citations discoverability surface.

The container-provable half of a labelled, host-assured nav walk: a single
sidebar nav entry that makes the S275 ``/verify-citations`` page discoverable,
on the S270 ``/claims`` nav idiom (itself the S248 ``/notes`` idiom). A
proven-ZERO add-only lot -- it edits one existing source (the sidebar
component) without flipping any canonical pin, and adds a test and a
host-assured runbook; the failing-name register stays byte-identical (the
tenth twin of the S266 register) and the version holds at 3.13.0.

Seven families, the S253 / S269 / S270 idiom:

1. the nav entry -- the ``/verify-citations`` object (href, label, icon) inside
   the ``navLinks`` array, placed after the ``/claims`` entry (RED on the
   pristine S275 tree, GREEN on the delivered tree);
2. the page seam -- the S275 ``/verify-citations`` page still renders
   ``CitationVerifier`` (a design-green premise, true on pristine);
3. the nav idiom -- the sidebar declares ``navLinks``, carries the ``/claims``
...
VERIFY_CITATIONS_PAGE = FRONTEND_SRC / "routes" / "verify-citations" / "+page.svelte"
SPEC = ROOT / "FRONTEND_REDESIGN_SPEC.md"
RUNBOOK = ROOT / "VERIFY_CITATIONS_NAV_E2E_S276.md"
...
# ---------------------------------------------------------------------------
# Family 1 -- the /verify-citations nav entry (RED on the pristine S275 tree)
# ---------------------------------------------------------------------------


class TestNavEntry:
    def test_verify_citations_href_present(self):
        assert "'/verify-citations'" in _read(SIDEBAR)

    def test_verify_citations_label_present(self):
        assert "label: 'Citations'" in _read(SIDEBAR)

    def test_verify_citations_entry_well_formed(self):
        src = _read(SIDEBAR)
        pattern = (
            r"\{\s*href:\s*'/verify-citations',\s*"
            r"label:\s*'Citations',\s*icon:\s*'[a-z0-9-]+'\s*\}"
        )
        assert re.search(pattern, src) is not None

    def test_verify_citations_entry_within_navlinks(self):
        region = _navlinks_region(_read(SIDEBAR))
        assert "'/verify-citations'" in region

    def test_verify_citations_placed_after_claims(self):
        src = _read(SIDEBAR)
        assert ("'/verify-citations'" in src) and (
            src.find("'/verify-citations'") > src.find("'/claims'")
        )


# ---------------------------------------------------------------------------
# Family 2 -- the page seam (design-green: S275 already ships the page)
# ---------------------------------------------------------------------------


class TestPageSeam:
    def test_verify_citations_page_renders_verifier(self):
        assert "CitationVerifier" in _read(VERIFY_CITATIONS_PAGE)
...
class TestNavIdiom:
    ...
    def test_sidebar_claims_entry_present(self):
        src = _read(SIDEBAR)
        assert ("'/claims'" in src) and ("label: 'Claims'" in src)
...
class TestRunbook:
    def test_exists_and_titled(self):
        assert _read(RUNBOOK).startswith("# VERIFY_CITATIONS_NAV_E2E_S276")

    def test_status_and_discipline(self):
        text = _flat(_read(RUNBOOK))
        assert "written at S276" in text
