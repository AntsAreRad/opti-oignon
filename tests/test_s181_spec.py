#!/usr/bin/env python3
"""Tests for S181 Goal 5 -- the cartography and docs (Theme 4 / Veilid Sync).

Validates that the S181 transport, producers, responder, and status surface are
recorded where the bloc's cartography requires:

- transport.py, producers.py, and sync_status.py are registered in VEILID_SPEC.md,
  and every module on disk under opti_oignon/veilid/ is registered (the on-disk
  invariant, re-asserted).
- VEILID_SPEC section 9 has been promoted: it names the three new modules and the
  status route, says the transport and status surface are built (S181), and the old
  "deferred to S181" wording is gone; section 6 lists S181 as built, not deferred;
  the keep-green S180 names (peers.py, sync_engine.py, routes_sync.py) and the Bulbe
  and approval statements survive, and the twelve-section structure is intact.
- The package __init__ exports the new S181 surface.
- The MkDocs page describes the live transport, the bidirectional exchange, and the
  sync-status surface while keeping the S178-S180 model tokens, the Bulbe boundary,
  and the optional install.
- ODYSSEUS_SPEC and FRONTEND_REDESIGN_SPEC stay green.
- The edits introduced no emojis, no equals-sign separator bars, and no decorative
  uppercase banners.

File-content only, so the suite collects without the backend.
"""

import re
from pathlib import Path

import pytest

ROOT = Path(__file__).resolve().parent.parent
SPEC = ROOT / "VEILID_SPEC.md"
VEILID_PKG = ROOT / "opti_oignon" / "veilid"
INIT = VEILID_PKG / "__init__.py"
ODYSSEUS_SPEC = ROOT / "ODYSSEUS_SPEC.md"
FRONTEND_SPEC = ROOT / "FRONTEND_REDESIGN_SPEC.md"
DOCS_PAGE = ROOT / "docs" / "sync" / "veilid-sync.md"

S181_MODULES = [
    "opti_oignon/veilid/transport.py",
    "opti_oignon/veilid/producers.py",
    "opti_oignon/veilid/sync_status.py",
]


def _spec() -> str:
    return SPEC.read_text(encoding="utf-8")


def _docs() -> str:
    return DOCS_PAGE.read_text(encoding="utf-8")


def _init() -> str:
    return INIT.read_text(encoding="utf-8")


def _section(n: int) -> str:
    text = _spec()
    start = text.index(f"## {n}. ")
    end = text.index(f"## {n + 1}. ", start)
    return text[start:end]


def _list_package_files() -> list[str]:
    names: list[str] = []
    if VEILID_PKG.is_dir():
        for p in VEILID_PKG.rglob("*.py"):
            names.append(p.relative_to(ROOT).as_posix())
    return names


# Cartography registration


class TestCartography:
    def test_s181_modules_registered(self):
        text = _spec()
        missing = [m for m in S181_MODULES if m not in text]
        assert not missing, f"S181 modules not registered in spec: {missing}"

    def test_all_on_disk_modules_registered(self):
        text = _spec()
        on_disk = _list_package_files()
        unregistered = [
            p for p in on_disk if p not in text and Path(p).name not in text
        ]
        assert not unregistered, f"On-disk modules not registered: {unregistered}"

    def test_new_artifacts_present_on_disk(self):
        for m in S181_MODULES:
            assert (ROOT / m).exists(), f"expected file missing: {m}"

    def test_s180_modules_still_registered(self):
        text = _spec()
        for m in (
            "opti_oignon/veilid/peers.py",
            "opti_oignon/veilid/sync_engine.py",
            "opti_oignon/api/routes_sync.py",
        ):
            assert m in text, f"S180 registration lost: {m}"


# Section 9 promoted to name the S181 work as built


class TestSection9Implemented:
    def test_section9_heading_present(self):
        assert "## 9. " in _spec()

    def test_section9_names_new_modules(self):
        s9 = _section(9)
        for name in ("transport.py", "producers.py", "sync_status.py"):
            assert name in s9, f"section 9 should name {name}"

    def test_section9_keeps_s180_artifacts(self):
        s9 = _section(9)
        for name in ("peers.py", "sync_engine.py", "routes_sync.py"):
            assert name in s9, f"section 9 should keep naming {name}"

    def test_section9_says_transport_and_status_built(self):
        s9 = _section(9).lower()
        assert "s181" in s9
        assert "built" in s9
        assert "transport" in s9 and "status" in s9

    def test_section9_names_status_route(self):
        assert "/api/sync/status" in _section(9)

    def test_section9_old_deferred_wording_gone(self):
        s9 = _section(9).lower()
        # the S180 deferral of the transport / status to S181 must be gone
        assert (
            "the live veilid transport driving the route over the node and client, "
            "and the route's status surface" not in s9
        )

    def test_section9_deferred_is_only_the_panel(self):
        s9 = _section(9).lower()
        assert "deferred: the pairing and sharing-control panel (s182)" in s9

    def test_section9_keeps_bulbe_and_approval(self):
        s9 = _section(9).lower()
        assert "bulbe" in s9
        assert "approval" in s9 or "manage_skills" in s9

    def test_twelve_sections_intact(self):
        text = _spec()
        missing = [f"## {i}. " for i in range(1, 13) if f"## {i}. " not in text]
        assert not missing, f"missing sections: {missing}"


# Section 6 lists S181 as built, not deferred


class TestSection6Updated:
    def test_section6_lists_s181_as_built(self):
        s6 = _section(6).lower()
        assert "s181" in s6
        # the old "deferred ... (S181)" line is gone
        assert (
            "the live veilid transport that drives the route over the node and "
            "client across a private route, and the route's status surface (s181)"
            not in s6
        )

    def test_section6_deferred_is_s182(self):
        s6 = _section(6).lower()
        assert "deferred to the named session" in s6
        assert "s182" in s6


# Package exports


class TestPackageExports:
    def test_init_exports_new_symbols(self):
        text = _init()
        for name in (
            "VeilidPeer",
            "resolve_live_peer",
            "serve_app_call",
            "decode_answer",
            "ClientRouteMessenger",
            "conversation_record",
            "memory_canonical_record",
            "memory_archive_record",
            "skill_record",
            "tombstone_record",
            "SyncStatusStore",
            "get_sync_status_store",
            "RoundOutcome",
        ):
            assert name in text, f"__init__ should export {name}"


# Docs page describes the transport, the exchange, and the status surface


class TestDocsModel:
    def test_page_exists_and_has_h1(self):
        first = _docs().lstrip().splitlines()[0]
        assert first.startswith("# ")

    def test_mentions_transport(self):
        text = _docs().lower()
        assert "live transport" in text
        assert "private route" in text

    def test_mentions_bidirectional_serve(self):
        text = _docs().lower()
        assert "serv" in text  # serve / serving / serves
        assert "bidirectional" in text or "both pull" in text

    def test_mentions_status_surface(self):
        text = _docs().lower()
        assert "sync status" in text or "status" in text
        assert "/api/sync/status" in _docs()

    def test_keeps_route_and_watermark(self):
        text = _docs().lower()
        for token in ("route", "watermark", "per-peer"):
            assert token in text, f"docs should keep {token!r}"

    def test_keeps_protocol_model_tokens(self):
        text = _docs().lower()
        for token in ("convergent", "last-writer-wins", "tombstone", "pull"):
            assert token in text, f"docs should keep {token!r}"

    def test_keeps_bulbe_and_install(self):
        text = _docs().lower()
        assert "bulbe" in text
        assert "opti-oignon[veilid]" in text


# Cross-specs stay green


class TestCrossSpecsGreen:
    def test_other_specs_exist(self):
        assert ODYSSEUS_SPEC.exists()
        assert FRONTEND_SPEC.exists()

    def test_veilid_not_folded_into_odysseus(self):
        odysseus = ODYSSEUS_SPEC.read_text(encoding="utf-8")
        assert "opti_oignon/veilid/" not in odysseus

    def test_odysseus_registry_still_named(self):
        odysseus = ODYSSEUS_SPEC.read_text(encoding="utf-8")
        assert "opti_oignon/agent/skills.py" in odysseus
        assert "opti_oignon/memory/" in odysseus


# Conventions hold after the edits


class TestConventions:
    EMOJI = re.compile(
        "[\U0001F300-\U0001FAFF"
        "\U00002600-\U000027BF"
        "\U0001F900-\U0001F9FF"
        "\U0001F600-\U0001F64F"
        "\U0001F680-\U0001F6FF]"
    )

    def test_no_emojis(self):
        for text, label in ((_spec(), "spec"), (_docs(), "docs")):
            m = self.EMOJI.search(text)
            assert m is None, f"emoji in {label} at {m.start()}" if m else ""

    def test_no_equals_bars(self):
        for text in (_spec(), _docs()):
            for line in text.splitlines():
                if re.fullmatch(r"=+", line.strip()) and len(line.strip()) >= 20:
                    pytest.fail(f"'=' banner found: {line[:40]!r}")

    def test_no_uppercase_banners_in_spec(self):
        for line in _spec().splitlines():
            stripped = line.strip()
            if (
                len(stripped) >= 8
                and stripped == stripped.upper()
                and re.fullmatch(r"[A-Z][A-Z\s]+", stripped)
                and not stripped.startswith(("|", "-", "`", "#"))
            ):
                pytest.fail(f"uppercase banner found: {stripped!r}")
