...
- Incremental update (re-indexes only modified files)
- Support de multiples types de fichiers
- Rich metadata (source, type, date, etc.)
"""
...
class DocumentIndexer:
    ...
    def __init__(
        ...
        # Cache of indexed files (hash -> metadata)
    ...
    def _load_index_cache(self) -> dict[str, dict]:
        """Load the indexing cache from disk."""
        if self._index_cache_file.exists():
            try:
                with open(self._index_cache_file, encoding='utf-8') as f:
                    return json.load(f)
    ...
    def _get_file_type(self, filepath: Path) -> str | None:
    ...
    def _read_file(self, filepath: Path) -> str | None:
        """
        Lit le contenu of a fichier avec management des encodages.

        Supporte les formats texte (UTF-8, Latin-1, etc.) et les formats
        binaries (PDF, Excel, DOCX) via specialized extraction.

        Args:
            filepath: File path to read

        Returns:
            Text content of the file, or None on error
        """
    ...
    def _extract_pdf(self, filepath: Path) -> str | None:
        """
    ...
    def _extract_excel(self, filepath: Path) -> str | None:
        """
        Extrait le texte of a fichier Excel.

        Args:
            filepath: File path Excel
    ...
    def _extract_docx(self, filepath: Path) -> str | None:
        """
        Extrait le texte of a fichier Word (.docx).

        Args:
            filepath: File path DOCX
    ...
    def index_file(
        ...
        file_type = self._get_file_type(filepath)
        if not file_type:
            logger.debug(f"Unsupported type: {filepath.suffix}")
            return (0, 1)
        ...
        if not force and not self._is_file_modified(filepath, file_hash):
            logger.debug(f"File not modified: {filepath}")
            return (0, 1)
        ...
        if not valid_chunks:
            logger.warning(f"No valid embedding for {filepath}")
            return (0, 1)
        ...
        logger.info(f"Indexed: {filepath.name} ({len(valid_chunks)} chunks)")
    ...
    def _remove_file_chunks(self, filepath: Path):
        ...
        try:
            ...
            if results and results['ids']:
                ...
                logger.debug(f"Deleted {len(results['ids'])} chunks from {filepath.name}")
        except Exception as e:
    ...
    def index_directory(
        ...
        Args:
            ...
            file_types: List of types to include (None = all)
...
if __name__ == "__main__":
    ...
    # If an argument is passed, index that folder
    if len(sys.argv) > 1:
