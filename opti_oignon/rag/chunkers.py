#!/usr/bin/env python3
"""
CHUNKERS - Intelligent document splitting
==============================================
Chunking strategies adapted to each file type.
...
class Chunk:
    ...
    total_chunks: int               # Total number of chunks from the file
    start_line: int | None = None   # Start line (if applicable)
    end_line: int | None = None     # Ligne de fin (si applicable)
    section_name: str | None = None # Nom de la section/fonction
...
class BaseChunker(ABC):
    ...
    def _split_with_overlap(self, chunks: list[str], overlap_chars: int) -> list[str]:
        ...
        for i, chunk in enumerate(chunks):
            if i == 0:
                # First chunk: add beginning of next
                if len(chunks) > 1:
                    next_start = chunks[1][:overlap_chars]
                    chunk = chunk + "\n...\n" + next_start
            elif i == len(chunks) - 1:
                # Last chunk: add end of previous
                prev_end = chunks[i-1][-overlap_chars:]
                chunk = prev_end + "\n...\n" + chunk
            else:
                # Middle chunks: add from both sides
                prev_end = chunks[i-1][-overlap_chars:]
...
class CodeChunker(BaseChunker):
    ...
    # Patterns for detecting Python structures
    ...
    def chunk(self, content: str, source_file: str, file_type: str) -> list[Chunk]:
        ...
        for i, (start_pos, name, block_type) in enumerate(boundaries):
            # Find end of block (start of next or end of file)
    ...
    def _find_boundaries(self, content: str) -> list[tuple[int, str, str]]:
        ...
        for match in self.FUNCTION_PATTERN.finditer(content):
            # Check that function is not indented (module level)
    ...
    def _chunk_by_size(
        ...
        """Split by size with smart separators."""
...
class RChunker(BaseChunker):
    ...
    def chunk(self, content: str, source_file: str, file_type: str) -> list[Chunk]:
        """Split R code by function/section."""
        chunks = []
...
class MarkdownChunker(BaseChunker):
    ...
    def chunk(self, content: str, source_file: str, file_type: str) -> list[Chunk]:
        ...
        for i, match in enumerate(headers):
            ...
            # End = next header of equal or higher level, or end
            end_pos = len(content)
        ...
        # Inclure le contenu avant le premier header if it existe
...
class CSVChunker(BaseChunker):
    """Chunker pour les fichiers CSV - garde le header avec each chunk."""

    def chunk(self, content: str, source_file: str, file_type: str) -> list[Chunk]:
        """Split CSV while keeping the header."""
...
class PDFChunker(BaseChunker):
    ...
    then splits intelligently respecting paragraphs.
    """

    def chunk(self, content: str, source_file: str, file_type: str) -> list[Chunk]:
        """
        Split PDF content (already extracted as text).

        Note: Le contenu est extrait par l'indexer avant d'arriver ici.
        We receive raw text, not binary PDF.
        """
        ...
        # Update type to indicate PDF origin
        for chunk in chunks:
            chunk.file_type = "pdf"
    ...
    def extract_text_from_pdf(filepath: str) -> str:
        """
        Extrait le texte of a fichier PDF.
...
class ExcelChunker(BaseChunker):
    ...
    Processes each sheet separately and keeps headers
    with each data chunk.
    """

    def chunk(self, content: str, source_file: str, file_type: str) -> list[Chunk]:
        """
        Split Excel content (already converted to text).
        ...
        # If content contains sheet markers, split by sheet
        if "[Sheet:" in content:
    ...
    def _chunk_by_sheets(self, content: str, source_file: str, file_type: str) -> list[Chunk]:
        """Split content by separating Excel sheets."""
        import re

        # Pattern to separate sheets: [Sheet: SheetName]
        ...
        chunks = []
        i = 1  # Data starts at index 1 (after the first match)
        ...
        while i < len(sheets):
            ...
            if sheet_content.strip():
                # Utiliser CSVChunker pour each feuille
                csv_chunker = CSVChunker(self.config)
    ...
    def extract_text_from_excel(filepath: str) -> str:
        """
        Extrait le texte of a fichier Excel.
        ...
        try:
            ...
            for sheet_name in wb.sheetnames:
                ...
                for row in sheet.iter_rows(values_only=True):
                    # Convert les valeurs en strings
                    row_str = [str(cell) if cell is not None else "" for cell in row]
                    # Ignore entirely empty lines
...
class DocxChunker(BaseChunker):
    ...
    def chunk(self, content: str, source_file: str, file_type: str) -> list[Chunk]:
        """
        Split DOCX content (already extracted as text).
    ...
    def extract_text_from_docx(filepath: str) -> str:
        """
        Extrait le texte of a fichier Word (.docx).
...
def get_chunker(file_type: str) -> BaseChunker:
    """Return the appropriate chunker for the file type."""
    chunkers = {
        "python": CodeChunker,
        "r": RChunker,
